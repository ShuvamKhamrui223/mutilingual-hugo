    ...
    },
    {
      ...
      value: function animateCSS(element, animation, reserved, callback) {
        ...
        (_element$classList = element.classList).add.apply(
          _element$classList,
          ["animate__animated"].concat(_toConsumableArray(animation))
        );

        var handler = function handler() {
          var _element$classList2;

          (_element$classList2 = element.classList).remove.apply(
            _element$classList2,
            ["animate__animated"].concat(_toConsumableArray(animation))
          );
  ...
  _createClass(Theme, [
    {
      ...
      value: function initRaw() {
        ...
        this.util.forEach(
          document.querySelectorAll("[data-raw]"),
          function ($raw) {
          ...
          function ($icon) {
            fetch($icon.getAttribute("data-svg-src"))
              ...
              .then(function (svg) {
                ...
                $svg.setAttribute(
                  "data-svg-src",
                  $icon.getAttribute("data-svg-src")
                );
              ...
              function () {
                ...
                window.localStorage &&
                  localStorage.setItem(
                    "theme",
                    _this2.isDark ? "dark" : "light"
                  );

                var _iterator = _createForOfIteratorHelper(
                    _this2.switchThemeEventSet
                  ),
                  _step;
    ...
    },
    {
      ...
      value: function initSearch() {
        ...
        if (
          ...
          return;
        var maxResultLength = searchConfig.maxResultLength
          ? searchConfig.maxResultLength
          : 10;
        var snippetLength = searchConfig.snippetLength
          ? searchConfig.snippetLength
          : 50;
        var highlightTag = searchConfig.highlightTag
        ...
        var $searchInput = document.getElementById(
          "search-input-".concat(suffix)
        );
        var $searchToggle = document.getElementById(
          "search-toggle-".concat(suffix)
        );
        var $searchLoading = document.getElementById(
          "search-loading-".concat(suffix)
        );
        var $searchClear = document.getElementById(
          "search-clear-".concat(suffix)
        );
        ...
        if (isMobile) {
          ...
          document.getElementById("search-cancel-mobile").addEventListener(
            ...
            function () {
              ...
              _this3._searchMobile &&
                _this3._searchMobile.autocomplete.setVal("");
            },
            false
          );
          ...
          $searchClear.addEventListener(
            ...
            function () {
              $searchClear.style.display = "none";
              _this3._searchDesktop &&
                _this3._searchDesktop.autocomplete.setVal("");
            },
            false
          );
        ...
        var initAutosearch = function initAutosearch() {
                  ...
                  var search = function search() {
                    ...
                    _this3._index.search(query).forEach(function (_ref) {
                      ...
                      if (position > 0) {
                        ...
                        context =
                          "..." + context.substr(position, snippetLength);
                      } else {
                      ...
                      Object.keys(metadata).forEach(function (key) {
                        title = title.replace(
                          new RegExp("(".concat(key, ")"), "gi"),
                          "<"
                            .concat(highlightTag, ">$1</")
                            .concat(highlightTag, ">")
                        );
              ...
              templates: {
                suggestion: function suggestion(_ref5) {
                  ...
                  return '<div><span class="suggestion-title">'
                    .concat(title, '</span><span class="suggestion-date">')
                    .concat(
                      date,
                ...
                empty: function empty(_ref6) {
                  ...
                  return '<div class="search-empty">'
                    .concat(
                      searchConfig.noResultsFound,
                      ': <span class="search-query">"'
                    )
                    .concat(query, '"</span></div>');
                },
                footer: function footer(_ref7) {
                  ...
                  return '<div class="search-footer">Search by <a href="'
                    .concat(href, '" rel="noopener noreffer" target="_blank">')
                    .concat(icon, " ")
                    .concat(searchType, "</a></div>");
                },
              },
            }
          );
          autosearch.on(
            ...
            function (_event, suggestion, _dataset, _context) {
              window.location.assign(suggestion.uri);
            }
          );
          if (isMobile) _this3._searchMobile = autosearch;
          else _this3._searchDesktop = autosearch;
        };
    ...
    },
    {
      ...
      value: function initDetails() {
        this.util.forEach(
          ...
          function ($details) {
            var $summary =
              $details.getElementsByClassName("details-summary")[0];
        ...
        this.util.forEach(
          ...
          function ($chroma) {
            ...
            if ($codeElements.length) {
              ...
              $header.className =
                "code-header " + $code.className.toLowerCase();
              ...
              if (
                _this5.config.code.maxShownLines < 0 ||
                code.split("\n").length < _this5.config.code.maxShownLines + 2
              )
                $chroma.classList.add("open");
          ...
          this.util.forEach(
            ...
            function ($header) {
              ...
              $header.insertAdjacentHTML(
                "afterbegin",
                '<a href="#'.concat($header.id, '" class="header-mark"></a>')
              );
            }
          );
        }
    ...
    },
    {
      ...
      value: function initToc() {
        ...
        if (
          ...
          $toc.style.maxWidth = "".concat(
            $page.getBoundingClientRect().left - 20,
            "px"
          );
          ...
          var $headerLinkElements =
            document.getElementsByClassName("headerLink");
          var headerIsFixed =
          ...
          this._tocOnScroll =
            ...
            function () {
              ...
              for (var i = 0; i < $headerLinkElements.length - 1; i++) {
                var thisTop =
                  $headerLinkElements[i].getBoundingClientRect().top;
                var nextTop =
    ...
    },
    {
      ...
      value: function initTypeit() {
        ...
        if (this.config.typeit) {
          ...
          var cursorSpeed = typeitConfig.cursorSpeed
      ...
      value: function initComment() {
        ...
        if (this.config.comment) {
          ...
          if (this.config.comment.giscus) {
            ...
            if (giscusConfig.lazyLoading)
              giscusScript.setAttribute("data-loading", "lazy");
            giscusScript.setAttribute(
              "data-theme",
              this.isDark ? giscusConfig.darkTheme : giscusConfig.lightTheme
            );
